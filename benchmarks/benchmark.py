...
def profile_cuda_mg(N, dtype, max_num_devices, verbose=False):
    ...
    try:
        cuda.empty_cache()
        workspace_elements = cusolver_mg_eigh_workspace_requirements(N, dtype, max_num_devices)
        workspace_bytes = workspace_elements * dtype.itemsize
        start = time.time()
